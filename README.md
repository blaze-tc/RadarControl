...
- `tests/`：85 项 .NET 自动化测试，以及 Unity 共享源码和内嵌 Bridge 完整性测试。
...
Release 全解决方案已达到 0 警告、85/85 自动测试通过。内嵌 `RadarBridge.exe` 已通过真实窗口启动和 Unity 父进程退出烟雾测试；UPM Runtime、Editor 和 Basic Interaction Sample 已在 Unity 2021.3.45f1 中完成真实编译验证。8 小时真机稳定性、真实场地标定与 Unity PlayMode/构建验收仍需现场执行；仓库已包含对应测试与检查清单。
