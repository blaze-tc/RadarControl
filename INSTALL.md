...
本文用于把 Blaze Radar SDK 安装到现有 Unity 项目，并验证包内自带的 `RadarBridge.exe` 可以正常启动。推荐使用固定版本 `v1.1.4`，便于团队成员和构建机获得完全相同的内容。
...
3. 安装下文的 `com.blaze.radar#v1.1.4`。
...
6. 在 Package Manager 中确认出现 **Blaze Radar SDK 1.1.4**。
...
{
  "dependencies": {
    "com.blaze.radar": "https://github.com/blaze-tc/RadarControl.git?path=/UnityPackage/com.blaze.radar#v1.1.4"
  }
}
```
...
- URL 中必须同时保留 `?path=/UnityPackage/com.blaze.radar` 和 `#v1.1.4`。
...
- [GitHub 标签 v1.1.4](https://github.com/blaze-tc/RadarControl/tree/v1.1.4)
